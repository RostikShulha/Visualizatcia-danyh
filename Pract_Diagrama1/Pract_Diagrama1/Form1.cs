    ...
    public partial class Form1 : Form
    {
        ...
        private void btCreateGist_Click(object sender, EventArgs e)
        {
            ...
            for (int j = 0; j < mGist; j++)
            {
                ...
                minYear++;
                if (C[j] < 0)
                { MessageBox.Show("Серед введених елементів е від'ємний", "Помилка"); }
            }
        ...
        private void btCreateRound_Click(object sender, EventArgs e)
        {
            ...
            for (int j = 0; j < mRound; j++)
            {
                ...
                B[j] = "Поверн. катег. (" + minAge.ToString() + ")";
                Sum += A[j];
                minAge++;
                if (A[j] <= 0)
                {
                    MessageBox.Show("Серед введених елементів присутні від'ємні або нульові", "Помилка");
                }
            }
            if (Sum != 100)
            {
                MessageBox.Show("Сума відсоткового співвідношення віку повинна дорівнювати 100%. Перевірте дані", "Помилка");
            }
            else
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < mGist; j++)
                    {
                        ...
                        dgvMasGist.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
